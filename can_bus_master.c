...
void ConfigureUART(void)
{
    ...
    UARTStdioConfig(0, 115200, 16000000);  // Initialize the UART baud rate speed
}
...
void InitCAN0(void)
{
    ...
    //
    g_ui8TXMsgData[0] = 48 + 0;
    g_ui8TXMsgData[1] = 48 + 1;
    g_ui8TXMsgData[2] = 48 + 2;
    g_ui8TXMsgData[3] = 48 + 3;
    g_ui8TXMsgData[4] = 48 + 4;
    g_ui8TXMsgData[5] = 48 + 5;
    g_ui8TXMsgData[6] = 48 + 6;
    g_ui8TXMsgData[7] = 48 + 7;
    g_sCAN0TxMessage.ui32MsgID = 0x341;
...
int main(void)
{
    ...
    ConfigureUART(); // Initialize the UART0
    InitCAN0(); // Initialize CAN0
    UARTprintf("\nProject: Can bus master... \n\n\r"); // Print welcome message
    while(1)
    {
        if(g_ui32ErrFlag != 0)
            CANErrorHandler();
        SysCtlDelay(25000000);
        CANMessageSet(CAN0_BASE, TXOBJECT, &g_sCAN0TxMessage, MSG_OBJ_TYPE_TX); //Send Message
        /* Update new message */
        g_ui8TXMsgData[0] = 48 + rand() % 10;
        g_ui8TXMsgData[1] = 48 + rand() % 10;
        g_ui8TXMsgData[2] = 48 + rand() % 10;
        g_ui8TXMsgData[3] = 48 + rand() % 10;
        g_ui8TXMsgData[4] = 48 + rand() % 10;
        g_ui8TXMsgData[5] = 48 + rand() % 10;
        g_ui8TXMsgData[6] = 48 + rand() % 10;
        g_ui8TXMsgData[7] = 48 + rand() % 10;
        g_sCAN0TxMessage.ui32MsgID = rand() % 10;
        g_sCAN0TxMessage.ui32MsgIDMask = 0;
        g_sCAN0TxMessage.ui32Flags = MSG_OBJ_TX_INT_ENABLE;
        g_sCAN0TxMessage.ui32MsgLen = sizeof(g_ui8TXMsgData);
        g_sCAN0TxMessage.pui8MsgData = (uint8_t *)&g_ui8TXMsgData;
